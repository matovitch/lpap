...
def _(
    ...
    config = default_decoder_training_config()
    selected_run_id = ""
    log_path = training_log_path(project_root, config)
    regularizer_metrics = validation_regularizer_metric_names(config)
...
def _(
    ...
    if not active_run_id:
        output = mo.vstack(
            [mo.md("No decoder runs have been logged yet."), mo.Html(recent_runs_table)]
        )
    else:
        ...
        try:
            ...
            gallery = mo.md(f"Decoder gallery unavailable: `{error}`")
        output = mo.vstack(
            [
                mo.md(
                ...
                mo.Html(
                    render_loss_history_svg(
                        history, validation_regularizer_metrics=regularizer_metrics
                    )
                ),
                gallery,
