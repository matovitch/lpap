...
def _(
    ...
    if run_options:
        ...
        run_picker = mo.ui.dropdown(
            options={"No image-to-energy runs": ""},
            value="No image-to-energy runs",
            label="Run",
            disabled=True,
        )
...
def _(
    ...
    if not active_run_id:
        output = mo.md("No image-to-energy runs have been logged yet.")
    else:
        ...
        try:
            ...
            gallery = mo.md(f"Image-to-energy gallery unavailable: `{error}`")
        output = mo.vstack(
            [
                mo.md(
