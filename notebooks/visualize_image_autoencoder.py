...
def _():
    ...
    from lpap.visualization_notebook import render_image_autoencoder_run_gallery
...
def _(mo):
    mo.md("""
    # Image autoencoder visualization
    """)
    return
...
def _(
    ...
    if run_options:
        ...
        run_picker = mo.ui.dropdown(
            options={"No image autoencoder runs": ""},
            value="No image autoencoder runs",
            label="Run",
            disabled=True,
        )
...
def _(
    ...
    if not active_run_id:
        output = mo.md("No image autoencoder runs have been logged yet.")
    else:
        ...
        try:
            ...
            gallery = mo.md(f"Image autoencoder gallery unavailable: `{error}`")
        output = mo.vstack(
            [
                mo.md(
