...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "molab-launch-image-energy-flow: unknown arg: $1" >&2
      exit 1
      ;;
  esac
done
...
if [[ -z "$target_steps" ]]; then
  echo "molab-launch-image-energy-flow: --target-steps is required" >&2
  exit 1
fi
...
from molab.jobs import launch_image_energy_flow_bg
...
if ${as_json} == 1:
    ...
    print(
        f"spawned pid={result['pid']} kind=image_energy_flow "
        f"target={result['target_steps']} bg_stem={result['bg_stem']}",
        flush=True,
    )
PY
