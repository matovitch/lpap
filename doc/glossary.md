...
- **Image-energy flow (`image_energy_flow`)**: A 1D bidirectional flow: images are at `t=-1`, a signed log-normal energy prior (`|e| = scale · exp(σ Z)`, coin-toss signs) is at `t=0`, and image reconstruction is at `t=+1`. It integrates image→energy over `[-1, 0]` and energy→image over `[0, 1]`.
